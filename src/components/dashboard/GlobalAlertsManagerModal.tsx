...
import { X, Bell, Trash2, Loader2, AlertCircle, Search, Sparkles, CheckCircle2, History, Crown } from "lucide-react"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          >
            ...
            {/* Content Switcher based on Tier */}
            {((session?.user as any)?.subscription_tier || 'free').toLowerCase() === 'free' ? (
              <div className="flex-1 flex flex-col items-center justify-center py-12 px-4 text-center relative overflow-hidden">
                {/* Glowing Crown Icon */}
                <div className="size-16 rounded-3xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 mb-6 shadow-[0_0_25px_rgba(245,158,11,0.2)] relative z-10">
                  <Crown className="w-8 h-8 animate-pulse" />
                </div>
                
                <h3 className="text-lg font-black uppercase tracking-[0.2em] text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-yellow-400 to-amber-500 mb-3 relative z-10">
                  Price Alerts Premium
                </h3>
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 max-w-[340px] leading-relaxed mb-8 relative z-10">
                  Setting and managing real-time stock price alerts requires a LITE or PRO plan. Upgrade your account to unlock unlimited price alerts!
                </p>
                
                <Link 
                  href="/subscription"
                  onClick={onClose}
                  className="px-10 py-4 rounded-full bg-gradient-to-r from-amber-400 to-amber-500 text-black text-[11px] font-black uppercase tracking-[0.25em] text-center shadow-[0_0_30px_rgba(245,158,11,0.25)] hover:shadow-[0_0_40px_rgba(245,158,11,0.45)] hover:scale-105 transition-all duration-300 relative z-10"
                >
                  Upgrade Plan
                </Link>
              </div>
                      ...
                      <AnimatePresence initial={false}>
                        {filteredAlerts.map((a) => (
                          ...
                          >
                            <div className="flex justify-between items-start gap-4">
                              <div className="min-w-0">
                                <span className={cn(
                                  ...
                                  {a.asset_type === 'MF' ? 'Mutual Fund' : a.asset_type === 'US_EQUITY' ? 'US Stock' : 'Stock'}
                                </span>
                                <h3 className="font-headline font-black text-white text-base mt-2.5 tracking-tight truncate leading-none">
                                  {a.symbol}
                                </h3>
                                {a.name && (
                                  <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider mt-1 truncate max-w-[180px]">
                                    {a.name}
                                  </p>
                                )}
                              </div>
                              ...
                              {!a.is_triggered && (
                                <button
                                  ...
                                  className="size-8 rounded-xl bg-red-500/[0.06] border border-red-500/10 flex items-center justify-center text-zinc-500 hover:text-rose-400 hover:border-red-500/30 transition-all duration-300"
                                >
                            ...
                            <div className="flex justify-between items-end border-t border-white/5 pt-3.5 mt-3.5">
                              <div>
